...
class JPC210:
  def __init__(self, data):
    ...
    assert self.magic == "JPAC2-10"
    self.num_particles = fs.read_u16(data, 8)
    self.num_textures = fs.read_u16(data, 0xA)
    self.tex_offset = fs.read_u32(data, 0xC)
    ...
    for particle_index in range(self.num_particles):
      particle = JPC210_Particle(data, offset)
  ...
  def import_particles_from_disk(self, input_directory):
    ...
    for jpa_path in all_jpa_file_paths:
      ...
      with open(jpa_path, "rb") as f:
        data = BytesIO(f.read())
      particle = JPC210_Particle(data, 0)
...
class JPC210_Particle:
  def __init__(self, jpc_data, particle_offset):    
    ...
    self.chunks = []
    self.chunk_by_type = {}
    chunk_offset = particle_offset + 0x8
...
class TDB1(J3DChunk):
  ...
  def read_texture_ids(self):
    self.texture_ids = []
    self.num_texture_ids = 2 # not 100% sure of this, but it seems correct from testing

    for texture_id_index in range(self.num_texture_ids):
      texture_id = fs.read_u16(self.data, 0x8 + texture_id_index * 2)
  ...
  def save_chunk_specific_data(self):
    ...
    for texture_id_index, texture_id in enumerate(self.texture_ids):
      fs.write_u16(self.data, 0x8 + texture_id_index * 2, texture_id)
